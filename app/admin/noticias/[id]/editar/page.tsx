...
export default function EditarNoticiaPage() {
  ...
  const fetchNoticia = async () => {
    try {
      const response = await api.get(`/noticias/${params.id}`);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await api.put(`/noticias/${params.id}`, formData);
  ...
  if (loadingData) {
    return (
      <div className="flex items-center justify-center h-64">
        <div className="text-gray-600">Cargando noticia...</div>
      </div>
    );
  }
    ...
    <div className="content-wrapper space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div className="flex-1">
          ...
          <h1 className="text-3xl font-bold text-dark-gray">Editar Noticia</h1>
        </div>
      </div>
